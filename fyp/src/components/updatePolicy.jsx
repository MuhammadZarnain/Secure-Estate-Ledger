...
export const UpdatePolicy = ({Admincontract}) => {
  const [policies, setPolicies] = useState([{ id: 1, title: '', description: '' }]);

  const handleAddPolicy = () => {
    setPolicies([...policies, { id: policies.length + 1, title: '', description: '' }]);
};
  ...
  const {title,description} = policies;
  const handleSavePolicy = async(e) =>{
    
    try{
      if(Admincontract){
        const pol = await Admincontract.addPolicy(title,description);
        await pol.wait();
        console.log("POLICY ADDED");
      }else{console.error("COntract issue");}
    }catch(err){
      console.error(err);
    }

  }
    const handleEditPolicy = (index, field, value) => {
      const updatedPolicies = [...policies];
            ...
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              ...
              <div className="flex flex-col items-stretch w-[84%] ml-5 max-md:w-full max-md:ml-0">
                <div className="flex w-full flex-col items-stretch max-md:max-w-full">
                  ...
                  <h1 className="text-2xl font-bold mb-4">Update Policies</h1>
            {policies.map((policy, index) => (
                <div key={index} className="mb-4">
                    <input
                        ...
                        value={policy.title}
                        onChange={(e) => handleEditPolicy(index, 'title', e.target.value)}
                        className="w-full border rounded-md p-2 mb-2"
                    />
                    <textarea
                        placeholder="Policy Description"
                        value={policy.description}
                        onChange={(e) => handleEditPolicy(index, 'description', e.target.value)}
                    ...
                    <button
                        onClick={() => handleSavePolicy(index)}
                        className="bg-red-500 text-white px-4 py-2 rounded-md mr-2"
                    >
                       Save
                    </button>
