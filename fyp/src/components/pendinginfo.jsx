...
const pinataSecretKey="371b874d23c09fda20625ed30017824921f450bf710994be36f1678592539e9c"
 // Assuming you have Web3.js installed

const LandInfoComponent = ({ landContract,landId }) => {
  ...
  useEffect(() => {
    ...
    const fetchPendingRequests = async () => {
      try {
        const result = await landContract.getPendingRequests();
...
useEffect(() => {
  ...
  const landInfo = pendingRequests.filter(request => request.Land_id === landId);

  return (
    <div className="bg-white shadow-md rounded-lg p-6 mt-6 mx-auto max-w-4xl">
    ...
    <div>
      ...
      <div className="flex items-center">
        ...
        <span className="w-2/3 text-gray-800">{landInfo[0].fullName}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Email:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].email}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Father's Name:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].fatherName}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">CNIC:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].cnic}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Permanent Address:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].permanentAddress}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Temporary Address:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].temporaryAddress}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Size:</strong>
        <span className="w-2/3 text-gray-800"> {landInfo[0].size}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Location:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].location}</span>
      </div>
      <div className="flex items-center">
        <strong className="w-1/3 text-gray-600">Owner:</strong>
        <span className="w-2/3 text-gray-800">{landInfo[0].owner}</span>
      </div>
  <br></br>
  <div className="container mx-auto p-7 items-center">
      ...
      <div className="flex flex-wrap justify-center gap-2 mx-auto">
        ...
        <div>No information found for Land ID: {landId}</div>
      )}
    </div>
